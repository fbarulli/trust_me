...
logger.debug(f"main: APP_DIR: {APP_DIR}")  # Log APP_DIR to confirm the directory
logger.debug(f"main: Files in APP_DIR: {os.listdir(APP_DIR)}") # Log directory contents
...
def load_csv_data(csv_path: str) -> Optional[pd.DataFrame]:
    ...
    """
    logger.info("load_csv_data: Starting")  # Log function start
    ...
    try:
        ...
        logger.exception(f"load_csv_data: An error occurred while loading the CSV: {e}", exc_info=True) # Include traceback
        st.error(f"An error occurred while loading the CSV: {e}")
        return None
    finally:
        logger.info("load_csv_data: Finished")  # Log function finish
...
def display_intro_section(df: Optional[pd.DataFrame]) -> None:
    ...
    """
    logger.info("display_intro_section: Starting")  # Log function start
...
Fabian : all companies/categories""", unsafe_allow_html=True)
    ...
    logger.info("display_intro_section: Finished")  # Log function finish


def main() -> None:
    """Main application function."""
    logger.info("main: Starting")  # Log function start
    logger.debug(f"main: Streamlit Script Working Directory: {os.getcwd()}")
    logger.debug(f"main: Python Path BEFORE modification: {sys.path}") # Log before modification
    sys.path.insert(0, APP_DIR) # Add streamlit_app directory to path
    logger.debug(f"main: Python Path AFTER modification: {sys.path}") # Log after modification
    ...
    selected_section = str(selected_section) # cast to string just to be sure
    ...
    if selected_section == "Intro":
        ...
        if hasattr(scraping_section, 'show_scraping_section'): # Check if function exists
            logger.debug("main: Found show_scraping_section in scraping_section module.")
            scraping_section.show_scraping_section() # Call function from scraping_section.py, **NO** passing df and DATA_LOADED
        else:
            logger.error("main: show_scraping_section NOT found in scraping_section module!")
            st.error("Error: Scraping section functionality not found. Please check the code.")
    ...
    logger.info("main: Finished")  # Log function finish
